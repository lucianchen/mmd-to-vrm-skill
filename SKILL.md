...
3. 依据实际**顶点权重**选择普通腿骨或 D 腿骨。检查两套同时有权重的情况、脚尖父骨、肩膀和扭转辅助骨。先理解附加旋转约束，再决定重接或合并；不要只按名字猜。
...
7. 原始导出保留为 `*-export-original.vrm`，最终文件用独立名称。运行 `scripts/optimize_vrm.mjs`；其支持范围是单蒙皮源网格、内嵌贴图的 VRM 1.0，遇到不支持的结构报错。它验证优化前后有效访问器数值及贴图字节，不能证明整个 Blender→VRM 过程无损。
...
- 源 PMX/BLEND、贴图、角色截图、机器专属配置和诊断报告保留在本地任务目录。发布本 skill 时提交可复用代码与文档；额外外部操作按用户授权执行。
