...
6. Inspect the scoped Git diff and staged filenames. In the owner-authorized maintenance repository, confirm the intended origin, commit the verified skill changes, push, and verify the remote commit and the owner's requested visibility (currently public). Do not force-push or alter other repositories. Public Pages deploys only the static viewer build; never add models, textures or local manifests to that artifact. When sync is not authorized or unavailable, keep the verified local improvement and state that remote sync remains outstanding.
