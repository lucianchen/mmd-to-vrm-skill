export function normalizeAssetPath(value){
  ...
  const parts=[];
  for(const part of decoded.replaceAll('\\','/').split('/')){if(!part||part==='.')continue;if(part==='..'){if(!parts.length)throw new Error('Asset escapes the selected folder');parts.pop();}else parts.push(part);}
  return parts.join('/');
}
export function resolveSelectedFile(files,url){
  ...
  throw new Error(folded.length?'Ambiguous texture path: '+key:'Missing selected texture: '+key);
}
