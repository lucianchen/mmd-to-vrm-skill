...
function createPane(kind){
  ...
  renderer.domElement.addEventListener('webglcontextlost',event=>{event.preventDefault();status('显存上下文已丢失，请刷新或换用较小模型。',true);});
  return pane;
}
function disposeRoot(pane){
  ...
  for(const url of pane.objectURLs){URL.revokeObjectURL(url);urls.delete(url);}pane.objectURLs=[];pane.root=pane.vrm=pane.mesh=null;pane.stats=null;delete messages[pane.kind];$(pane.kind+'-empty').hidden=false;$(pane.kind+'-name').textContent='正在载入…';$(pane.kind+'-stats').textContent='读取几何与贴图';
}
...
function mount(pane,root,name,bytes,poseNote){
  root.updateMatrixWorld(true);const bounds=new THREE.Box3().setFromObject(root,true),size=bounds.getSize(new THREE.Vector3());if(!Number.isFinite(size.y)||size.y<.00001)throw new Error('模型没有有效高度');
  ...
  pane.bytes=bytes;pane.stats={triangles,vertices,pose:poseNote};$(pane.kind+'-name').textContent=name;$(pane.kind+'-stats').textContent=triangles.toLocaleString()+' 三角形 · '+(bytes/1e6).toFixed(1)+' MB';$(pane.kind+'-empty').hidden=true;messages[pane.kind]=poseNote;frame(currentView);status('已载入 · '+Object.entries(messages).map(([k,v])=>k.toUpperCase()+' '+v).join(' / '));
}
function managerFor(resolver){
  let done,fail;const complete=new Promise((a,b)=>{done=a;fail=b;}),manager=new THREE.LoadingManager(done,undefined,url=>fail(new Error('资源加载失败：'+url)));manager.setURLModifier(url=>{if(url.startsWith('data:')||(url.startsWith('blob:')&&new URL(url).origin===location.origin))return url;return resolver(url);});return {manager,complete};
}
async function loadMmd(url,name,bytes,resolver,objectURLs=[]){
  const pane=panes.mmd;disposeRoot(pane);pane.objectURLs=objectURLs;status('正在读取 MMD 与贴图…');
  ...
  const aligned=alignMmdArms(mesh);mount(pane,mesh,name,bytes,aligned?'T 姿态':'源姿态（未识别手臂骨骼）');
}
async function loadVrm(url,name,bytes,resolver,objectURLs=[]){
  const pane=panes.vrm;disposeRoot(pane);pane.objectURLs=objectURLs;status('正在读取 VRM…');
  const {manager,complete}=managerFor(resolver),loader=new GLTFLoader(manager);loader.register(p=>new VRMLoaderPlugin(p));
  const [gltf]=await Promise.all([loader.loadAsync(url),complete]);const vrm=gltf.userData.vrm;if(!vrm)throw new Error('文件没有 VRM 扩展');
  VRMUtils.rotateVRM0(vrm);vrm.humanoid.resetNormalizedPose();vrm.update(0);pane.vrm=vrm;mount(pane,vrm.scene,name,bytes,'T 姿态');
}
function schedule(fn){loadQueue=loadQueue.then(fn).catch(error=>{console.error(error);status(error.message,true);});return loadQueue;}
...
$('mmd-input').addEventListener('change',event=>{selectedFiles=new Map([...event.target.files].map(f=>[normalizeAssetPath(f.webkitRelativePath||f.name),f]));const models=[...selectedFiles.keys()].filter(k=>/\.pmx$/i.test(k));const select=$('pmx-select');select.replaceChildren(...models.map(key=>new Option(key.split('/').pop(),key)));$('model-choice').hidden=models.length<2;if(!models.length){status('选择的文件夹中没有 PMX 文件。',true);return;}schedule(()=>loadSelectedMmd(models[0]));});
$('pmx-select').addEventListener('change',e=>schedule(()=>loadSelectedMmd(e.target.value)));
$('vrm-input').addEventListener('change',event=>{const file=event.target.files[0];if(!file)return;schedule(async()=>{const url=objectURL(file);await loadVrm(url,file.name,file.size,v=>{if(v===url)return v;throw new Error('VRM 使用了外部资源，请导出贴图内嵌版本。');},[url]);});});
...
$('export').addEventListener('click',()=>{if(!panes.mmd.root||!panes.vrm.root){status('请先载入两侧模型，再导出对比图。',true);return;}render();const canvas=document.createElement('canvas'),width=1200,height=840;canvas.width=width;canvas.height=height;const c=canvas.getContext('2d');c.fillStyle='#fafaf7';c.fillRect(0,0,width,height);c.fillStyle='#232a30';c.font='600 20px sans-serif';for(const [i,p] of Object.values(panes).entries()){c.fillText(i?'VRM / CONVERTED':'MMD / SOURCE',i*600+24,38);const source=p.renderer.domElement,s=Math.min(600/source.width,750/source.height),w=source.width*s,h=source.height*s;c.fillStyle='#eeece7';c.fillRect(i*600,56,600,750);c.drawImage(source,i*600+(600-w)/2,56+(750-h)/2,w,h);}c.fillStyle='#737b81';c.font='12px sans-serif';c.fillText('MMD Toon / MToon · Same camera & lighting · Height normalized · Physics paused',24,826);canvas.toBlob(blob=>{const url=objectURL(blob),a=document.createElement('a');a.href=url;a.download='mmd-vrm-comparison.png';a.click();setTimeout(()=>{URL.revokeObjectURL(url);urls.delete(url);},1000);});status('对比图已导出。');});
...
createPane('mmd');createPane('vrm');let last=performance.now();
...
window.comparison={panes,frame,render,ready:false,get state(){return Object.fromEntries(Object.entries(panes).map(([k,p])=>[k,{loaded:!!p.root,stats:p.stats,position:p.camera.position.toArray(),target:p.controls.target.toArray()}]));}};
window.addEventListener('beforeunload',()=>urls.forEach(url=>URL.revokeObjectURL(url)));
async function start(){const manifest=await fetch('/example.json').then(r=>r.json());if(manifest.mmd){await schedule(()=>loadMmd(manifest.mmd.url,manifest.mmd.name,manifest.mmd.bytes,url=>{const resolved=new URL(url,location.href);if(resolved.origin!==location.origin||!resolved.pathname.startsWith('/model-assets/'))throw new Error('MMD 资源超出所选目录');return resolved.href;}));}if(manifest.vrm){await schedule(()=>loadVrm(manifest.vrm.url,manifest.vrm.name,manifest.vrm.bytes,url=>{const resolved=new URL(url,location.href);if(resolved.origin!==location.origin||resolved.pathname!=='/example.vrm')throw new Error('VRM 外部资源不受支持');return resolved.href;}));}window.comparison.ready=true;}
